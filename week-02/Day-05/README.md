...
for item in list:
means:
Take each item from the list one at a time.
 
⸻
 
6. 
...
while not value.isdigit():
means:
Continue while the input is not valid digits.
...
count += 1
Use a counter when:
You want to know how many items satisfy a condition.
Example:
...
You want to calculate a total/result across multiple iterations.
 
⸻
 
...
for item in items:
    count = 0
This resets the counter every iteration.
Correct:
count = 0
...
for
→ range/sequence eventually ends
...
↓
while not condition:
The key to loops is not memorizing syntax.

Understand what controls the repetition, what changes during each iteration, and what eventually makes the loop stop.
